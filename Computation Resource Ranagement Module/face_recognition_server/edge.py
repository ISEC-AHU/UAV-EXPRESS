...
def receive_name():
    ...
    # 先与终端连接 接收 收货人姓名
    print('正在连接终端')
    tcp_client1.bind(('172.19.5.18', 2000))  # 边自己的ip
...
def receive_img():
    ...
    print('终端地址', client_addr)
    # 接收图片大小
    img_size = sock_client.recv(1024*1024)
    print('图片size:', int(img_size))
    sock_client.send('edge_ok'.encode())
    count = 0

...
import matplotlib.pyplot as plt # ZBW添加
import cv2  # ---以下GH 添加包
import client_zbw_v1
...
class YOLO(object):
    ...
    def detect_image(self, image):
        ...
        检测图像
        ZBW 修改
        :param image: python PIL Image 对象
        :return: 对象本身，识别出的目标数
        """
        start = timer()  # 开始计时
        ...
        for i, c in reversed(list(enumerate(out_classes))):
            ...
            # 存储分割后的图像
            img = self.cut_image(image, top, bottom, left, right)
            self.objs.append(img)
            # img.show()
            # client_zbw.send_img(img)
    ...
    def cut_image(self, image, top, bottom, left, right):
        """
        ZBW add
        :param image: PIL image object, not numpy array
        :param top: top coord of image
        :param bottom: bottom coord of image
        :param left: left coord of image
        :param right: right coord of image
        :return: divided sub image object
        """
    ...
    def send_image(self):
        ...
        for i in range(len(self.objs)):
            print(i)
            img_path = 'result_divided/temp'+str(i)+'.jpg'
            client_zbw_v1.send_img(self.objs[i], img_path)
        self.objs.clear()
...
def detect_images():
    ...
    for filename in os.listdir(path):
        ...
        r_image, reason_number = yolo.detect_image(image)  # GH 这里添加返回值 检测人数 以判断有无人
        yolo.send_image()
        file.write('\n')
        # r_image.show() # 显示检测结果
...
if __name__ == '__main__':
    client_zbw_v1.send_name()  # 发送收货人姓名
    signal = input('无人机到达地点 给出指令\n>>>')  # 无人机到达地点 给出指令
    if signal == '1':
        t1 = time.time()  # 开始收货计时
        detect_images()
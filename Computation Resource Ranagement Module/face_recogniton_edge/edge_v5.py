...
"""
ctx
尝试将所有功能放入receive_img()，为了让端确认才发送下一张
发送确认信息 'recognition_recv' 表示所有功能结束
"""
...
if __name__ == '__main__':
    ...
    while True:  # GH添加
        receive_img()
        print('开始接收图片，开始计时')
        # img_path, coords, img_name = receive_img()  # 接受检测后的图像、分割坐标、图像名
        # img_paths = divide_image(img_path, coords, img_name) # 切分图像并保存
        print('开始姿势识别')
